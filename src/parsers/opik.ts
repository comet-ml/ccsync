import { ClaudeMessage, OpikSpan, OpikTrace } from '../types';
...
export function claudeToOpikTraces({
  messages,
  startFromIndex = 0,
  ...
  for (const group of groups) {
    ...
    let lastSpanTime = userMessage.timestamp;;

    for (const message of assistantMessages) {
      const inputTokens = message.message?.usage?.input_tokens ?? 0;
      const outputTokens = message.message?.usage?.output_tokens ?? 0;
      const output = buildTimelineOutput([message]);
      const span: OpikSpan = {
        id: generateUUIDv7(),
        project_name: projectName,
        name: truncateString(output, 100),
        trace_id: trace.id,
        start_time: lastSpanTime,
        ...
        type: (message.toolUseResult) ? 'tool' : 'llm',
        provider,
      ...
      if (message.timestamp) {
        lastSpanTime = message.timestamp;
      }
    }
  }
...
function calculateTotalTokens(messages: ClaudeMessage[]): {
  input: number,
  output: number,
} {
  let inputTokens = 0;
